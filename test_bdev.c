...
static int test_bdev_parse_arg(int ch, char *arg)
{
    switch (ch)
    {
    case 'b':
...
static void test_bdev_usage(void)
{
...
static void test_bdev(void *arg)
{
    struct my_context *p = arg;
    uint32_t buf_align;
    uint32_t block_size;
...
int main(int argc, char **argv)
{
    ...
    opts.name = "test_bdev";
    rc = spdk_app_parse_args(argc, argv, &opts, "b:", NULL, test_bdev_parse_arg, test_bdev_usage);
    ...
    rc = spdk_app_start(&opts, test_bdev, &context);
    if (rc)
    {
